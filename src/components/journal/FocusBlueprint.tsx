...
export const FocusBlueprint: React.FC<FocusBlueprintProps> = ({
  ...
  const blocksCount = sessionsSnapshot?.length || 0;
  const todosCount = sessionsSnapshot?.reduce((acc, s) => acc + (s.todos?.filter(t => t.completed).length || 0), 0) || 0;
  const canvasCount = boardSnapshots?.length || 0;
  const windDownsCount = windDownSnapshot?.length || 0;

  const summaryParts = [];
  if (durationStr) summaryParts.push(`⏱️ ${durationStr}`);
  if (blocksCount > 0) summaryParts.push(`📦 ${blocksCount} block${blocksCount > 1 ? "s" : ""}`);
  if (todosCount > 0) summaryParts.push(`✓ ${todosCount} task${todosCount > 1 ? "s" : ""}`);
  if (canvasCount > 0) summaryParts.push(`🎨 ${canvasCount} canvas`);
  if (windDownsCount > 0) summaryParts.push(`🧘 ${windDownsCount} wind-down${windDownsCount > 1 ? "s" : ""}`);

  const summaryText = summaryParts.length > 0 ? summaryParts.join("  ·  ") : "No activities tracked";

  return (
    <div className="focus-blueprint-section" style={{ transition: "all 0.3s ease" }}>
      <div 
        className="focus-blueprint-header" 
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
          <span>✦ DAILY FOCUS BLUEPRINT</span>
          <span style={{ fontSize: "10.5px", fontWeight: "normal", opacity: 0.8, fontFamily: "var(--font-mono)", letterSpacing: "normal" }}>
            ({summaryText})
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "4px", fontFamily: "var(--font-mono)", fontSize: "10px", fontWeight: "bold" }}>
          <span>{isExpanded ? "Hide Details ▲" : "Show Details ▼"}</span>
        </div>
      </div>

      {isExpanded && (
        <div style={{ animation: "fadeUp 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards" }}>
